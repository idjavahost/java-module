...
class Java_ico {
	 ...
	 *
	 * If the constructor is not passed a file, a file will need to be supplied using the {@link Java_ico::add_image}
	...
	public function __construct( $file = false, $sizes = array() ) {
		...
		foreach ( $required_functions as $function ) {
			if ( ! function_exists( $function ) ) {
				log_message("error", "The Java_ico class was unable to find the $function function, which is part of the GD library. Ensure that the system has the GD library installed and that PHP has access to it through a PHP interface, such as PHP's GD module. Since this function was not found, the library will be unable to create ICO files.");
				return;
			}
		}
	...
	public function save_ico( $file ) {
		if ( ! $this->_has_requirements ) {
            log_message('error', 'Java_ico::save_ico() Requirements not meet.');
            return false;
        }

		if ( false === ( $data = $this->_get_ico_data() ) ) {
            log_message('error', 'Java_ico::save_ico() Icon data empty.');
            return false;
        }

		if ( false === ( $fh = fopen( $file, 'w' ) ) ){
            log_message('error', 'Java_ico::save_ico() Icon destination not writeable.');
            return false;
        }

		if ( false === ( fwrite( $fh, $data ) ) ) {
            log_message('error', 'Java_ico::save_ico() Failed to create icon image to destination.');
			fclose( $fh );
			return false;
		}
